...
// PUT /api/clientes/:id/categoria -> troca só a categoria do cliente (usado na tela de Instalação,
...
export async function onRequestPut(context: EventContext<Env, { id: string }, unknown>) {
  ...
  await env.DB.prepare('UPDATE clientes SET categoria_id = ? WHERE id = ?').bind(body.categoria_id || null, params.id).run();
