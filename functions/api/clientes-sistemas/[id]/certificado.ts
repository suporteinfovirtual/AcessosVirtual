...
export async function onRequestGet(context: EventContext<Env, { id: string }, unknown>) {
  ...
  const registro = await env.DB
    .prepare('SELECT nome_arquivo, arquivo FROM certificados_sistemas WHERE cliente_sistema_id = ?')
    .bind(params.id)
...
export async function onRequestPost(context: EventContext<Env, { id: string }, unknown>) {
  ...
  await env.DB
    .prepare(
      `INSERT INTO certificados_sistemas (cliente_sistema_id, nome_arquivo, arquivo, senha, validade, atualizado_em)
       VALUES (?, ?, ?, ?, ?, datetime('now'))
       ON CONFLICT(cliente_sistema_id) DO UPDATE SET
